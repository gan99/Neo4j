...
document.addEventListener('DOMContentLoaded', function() {
    ...
    cy.on('dbltap', 'node', async function(evt) {
        ...
        // Pass the current query set name to the neighbors endpoint so that the server
        // can select the appropriate neighbor definition.  Fallback to default_graph
        // if currentQuery.name is undefined.
        const querySetParam = currentQuery.name ? `&query_set_name=${encodeURIComponent(currentQuery.name)}` : '';
        const neighborsUrl = NEIGHBORS_API_URL_TEMPLATE.replace('{node_id}', nodeId) +
          `?limit=15&node_type=${nodeType}${querySetParam}`;
        // Fetch neighbour data and add it to the graph.  Store the collection of
        // newly added elements so we can position them manually.
        const newElements = await fetchDataAndRender(neighborsUrl);
        calculateRelativeSizes();
        // Position newly added nodes around the clicked node instead of running a
        // full layout, which avoids the graph “reloading” effect.
        if (newElements && newElements.length > 0) {
            const newNodes = newElements.nodes();
            // Place new nodes evenly around the clicked node in a small circle.
            const center = node.position();
            const radius = Math.max(parseInt(edgeLengthSlider.value), 50);
            const angleStep = (2 * Math.PI) / Math.max(newNodes.length, 1);
            let idx = 0;
            newNodes.forEach(n => {
                // Do not reposition the clicked node itself
