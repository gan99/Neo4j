...
/* ================= EDGE CAPTION PROPERTY SUPPORT =================
   Reads `edge_caption_property` (or `query_caption_property`) from the
   active query config and labels edges using that property key.
==================================================================== */
(function addEdgeCaptionSupport() {
  const header = document.querySelector('.header') || document.body;

  if (!document.getElementById('edge-caption-key')) {
    ...
    lbl.textContent = 'Edge label key:';

    const input = document.createElement('input');
    input.type = 'text';
    input.id = 'edge-caption-key';
    input.placeholder = 'name';
    input.value = window.currentEdgeCaptionProp || '';

    input.addEventListener('change', () => {
      window.currentEdgeCaptionProp = input.value.trim() || 'name';
      if (window.cy) applyEdgeLabels(window.cy, window.currentEdgeCaptionProp);
    });
  ...
  function applyEdgeLabels(cy, key) {
    try {
      cy.batch(() => {
        cy.edges().forEach(e => {
          const val = (key && e.data(key) != null && String(e.data(key)).trim() !== '')
            ? String(e.data(key))
            : (e.data('name') != null ? String(e.data('name')) : e.data('type') || e.data('label') || e.data('rel') || e.data('relationship') || e.data('~:type') || '');
          e.data('__edge_label__', val);
        });
      });
      const st = cy.style()
        .selector('edge')
        .style({
          'label': 'data(__edge_label__)',
      ...
      console.warn('applyEdgeLabels failed:', err);
    }
  }

  const origFetch = window.fetchDataAndRender;
  window.fetchDataAndRender = async function(url) {
    const res = await origFetch.apply(this, arguments);
    try {
      const inferredKey = (
        (window.currentQuery && (window.currentQuery.edge_caption_property || window.currentQuery.query_caption_property)) ||
        document.getElementById('edge-caption-key')?.value ||
        'name'
      );
      window.currentEdgeCaptionProp = inferredKey;
      if (window.cy) applyEdgeLabels(window.cy, inferredKey);
    } catch (e) {
      console.warn('edge caption hook failed:', e);
    }
    return res;
  };

  window.applyEdgeLabels = (key) => {
    window.currentEdgeCaptionProp = key || window.currentEdgeCaptionProp || 'name';
    if (window.cy) applyEdgeLabels(window.cy, window.currentEdgeCaptionProp);
  };
})();
/* =============== END EDGE CAPTION PROPERTY SUPPORT =============== */
