...
const seedElementIds = new Set();
const ownerRefCount  = new Map();
const expansionsByNode = new Map();
...
cy.on('dbltap', 'node', async function(evt) {
  ...
  // Maintain breadcrumb history (unchanged logic)
  ...
  if (!isExpanded) {
    ...
    // position new nodes around anchor
    const center = node.position();
    const newNodes = added.filter('node').filter(n => n.id() !== nodeId);
    const R = 90, N = Math.max(1, newNodes.length);
    ...
    if (reg) {
      // edges, then nodes
      // release
      const edgeIds = Array.from(reg.edgeIds);
      const nodeIds = Array.from(reg.nodeIds);
      // decrement and remove where count hits zero
      edgeIds.forEach(id => {
        const el = cy.getElementById(id);
        if (el.empty() || seedElementIds.has(id)) return;
        const n = (ownerRefCount.get(id) || 1) - 1;
