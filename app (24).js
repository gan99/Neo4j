...
cy.on('dbltap', 'node', async function(evt) {
        // Cancel single-tap property panel
        clearTimeout(tapTimeout);
        ...
        // If node already expanded -> collapse just its expansion
        ...
        // ---- expand: rebuild breadcrumb history up to this label ----
        ...
        // Track elements that were just added by this expansion
        const ids = new Set();
        if (added && added.forEach) {
            added.forEach(ele => {
                try { ids.add(ele.id()); } catch (_) {}
            });
        }
        // Also mark immediate new neighborhood (in case cy merged duplicates)
        node.neighborhood().forEach(ele => {
            ids.add(ele.id());
        });
        __expansionByParent.set(nodeId, ids);
