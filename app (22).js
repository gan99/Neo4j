...
cy.on('dbltap', 'node', async function(evt) {
        ...
        if (node.scratch && node.scratch('_expanded')) {
        ...
        // Maintain breadcrumb UI (not used for API unless flag is on)
        ...
        let neighborsUrl = NEIGHBORS_API_URL_TEMPLATE.replace('{node_id}', nodeId) 
            + `?limit=12&node_type=${encodeURIComponent(nodeType)}&query_key=${encodeURIComponent(document.getElementById('current-query-key')?.value || '')}&months=${months}`;

        // Deterministic neighbors: only append history if explicitly enabled
        if (USE_HISTORY_IN_NEIGHBOR_REQUEST && historyParams) {
            neighborsUrl += `&${historyParams}`;
        }

        // ------ EXPAND: snapshot -> fetch -> diff -> ownership -> collision-free placement ------
        ...
        node.scratch('_expansionCol', cy.collection(__addedAll));
        node.addClass('expanded');

        // Collision-free placement: ring + spiral until min distance from existing nodes
        if (__addedNodes.length > 0) {
            const center = node.position();
            const R0 = 140;
            const N  = Math.max(1, __addedNodes.length);
            const minGap = 46;
            const angleStep = (2 * Math.PI) / N;

            const existing = cy.nodes().filter(n => !__addedNodes.some(a => a.id() === n.id()));
            ...
            cy.startBatch();
            for (let i = 0; i < __addedNodes.length; i++) {
                const n = __addedNodes[i];
                let theta = angleStep * i;
                let radius = R0;
                let pos = { x: center.x + radius * Math.cos(theta), y: center.y + radius * Math.sin(theta) };
                ...
                while (tries < maxTries) {
                    ...
                    for (let j = 0; j < existing.length; j++) {
                        const p2 = existing[j].position();
                        if (dist2(pos, p2) < minGap2) { ok = false; break; }
                    }
                    if (ok) {
                        for (let k = 0; k < i; k++) {
                            const prev = __addedNodes[k].position();
                            if (dist2(pos, prev) < minGap2) { ok = false; break; }
                        }
                    }
                    if (ok) break;
                    theta += 0.6;
                    radius += 10;
                    pos = { x: center.x + radius * Math.cos(theta), y: center.y + radius * Math.sin(theta) };
                    tries++;
                }
                n.position(pos);
            }
