...
cy.on('dbltap', 'node', async function(evt) {
    ...
    // COLLAPSE: if this node has recorded expansion, remove ONLY what it added
    if (__expansionByParent.has(nodeId)) {
        ...
        ids.forEach(eid => {
            ...
            if (ele && ele.nonempty && ele.nonempty() && ele.isEdge()) {
        ...
        ids.forEach(eid => {
            const ele = cy.getElementById(eid);
            if (ele && ele.nonempty && ele.nonempty() && ele.isNode()) {
    ...
    // EXPAND: preserve your breadcrumb logic
    ...
    if (historyParams) neighborsUrl += `&${historyParams}`;

    // Run and capture only truly new elements
    const newIds = await __runExpansionAndGetNew(cy, async () => {
        const added = await fetchDataAndRender(neighborsUrl);
        return added;
    });

    if (newIds.length === 0) {
        // No new neighbors; don't mark as expanded
        return;
    }
