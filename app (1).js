...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function parseElements(elements, anchorNode) {
        ...
        if (anchorNode && newNodes.length > 0) {
            /*
             * Position newly added nodes in a ring around the anchor node.  To
             * reduce overlap when there are many neighbours, scale the radius
             * with the number of new nodes.  The base radius is derived from
             * the edge-length slider (minimum of 50) and we add 20 pixels per
             * neighbour to space them out further.
             */
            const baseRadius = Math.max(parseInt(edgeLengthSlider.value), 50);
            const radius = baseRadius + Math.max(newNodes.length - 1, 0) * 20;
