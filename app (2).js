...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function expandNode(node) {
        ...
        fetchDataAndRender(neighborsUrl, node).then(() => {
            // For force-based layouts, restart the simulation to allow new nodes to settle
            if (isForceLayout()) {
                simulation.alpha(1).restart();
            }
            // Do not call applyLayout for non-force layouts (circle, concentric, tree) after expansion
        });
    }
