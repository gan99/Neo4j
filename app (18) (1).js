console.info('UDM-Graph build 2025-09-16-2');
// app/static/app.js
...
function setActiveQueryKey(key){ const i=document.getElementById('current-query-key'); if(i){ i.value = key || ''; } }
    ...
    // Behavior flags
    const SINGLE_SOURCE_MODE = true;
    const USE_HISTORY_IN_NEIGHBOR_REQUEST = false;
    ...
    cy.on('dbltap', 'node', async function(evt) {
        ...
        // --- Toggle: collapse if this node is already expanded ---
        if (node.scratch('_expanded')) {
            ...
            if (col && col.length) {
                ...
                // Fallback to ID list if collection is missing
                const toRemoveIds = node.scratch('_expansionIds') || [];
        ...
        // Disabled to ensure only the clicked node's neighbors are requested
        // if (historyParams) {
        //     neighborsUrl += `&${historyParams}`;
        // }
        // In single-source mode, collapse other expanded nodes before expanding this one
        ...
        // Snapshot existing element IDs BEFORE adding
        const __beforeIds = new Set(cy.elements().map(e => e.id()));

        // Fetch & render neighbors
        await fetchDataAndRender(neighborsUrl);
        calculateRelativeSizes();

        // Compute truly new elements (so we don't disturb existing layout)
        const __addedAll   = cy.elements().filter(e => !__beforeIds.has(e.id()));
        // Keep a Cytoscape collection for robust collapsing even if IDs change order
        const __expansionCol = cy.collection(__addedAll);
        node.scratch('_expansionCol', __expansionCol);
        const __addedNodes = __addedAll.nodes().filter(n => n.id() !== nodeId);

        // Assign ownership so multiple expansions can co-exist
        const __ownerId = node.id();
        __addedAll.forEach(el => {
            const owners = Array.isArray(el.data('_owners')) ? el.data('_owners') : [];
            if (!owners.includes(__ownerId)) {
                owners.push(__ownerId);
                el.data('_owners', owners);
            }
        });

        // Save expansion state (list of ids) on the node
        node.scratch('_expanded', true);
        node.scratch('_expansionIds', __addedAll.map(e => e.id()));
        node.addClass('expanded');

        // Prevent merged/stacked nodes: ring placement + small local layout
        if (__addedNodes.length > 0) {
            const __center = node.position();
            const __R = 120;
            const __N = Math.max(1, __addedNodes.length);

            // Initial ring placement
            __addedNodes.positions((i, n) => {
                const theta = (2 * Math.PI * i) / __N;
                return { x: __center.x + __R * Math.cos(theta), y: __center.y + __R * Math.sin(theta) };
            });

            // Tiny jitter so identical positions never stack
            __addedNodes.forEach(n => {
                const p = n.position();
                n.position({ x: p.x + (Math.random()-0.5)*4, y: p.y + (Math.random()-0.5)*4 });
            });

            // Local, bounded layout just for the newly added nodes (no global jump)
            const __BB = { x1: __center.x - 160, y1: __center.y - 160, x2: __center.x + 160, y2: __center.y + 160 };
            if (__N >= 3) {
              __addedNodes.layout({
                  name: 'circle',
                  fit: false,
