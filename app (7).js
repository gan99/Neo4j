...
// --- Expansion bookkeeping for dbl-click toggle ---
const seedElementIds = new Set();                 // elements from the initial (primary) query
const ownerRefCount  = new Map();                 // elementId -> how many expansions include it
const expansionsByNode = new Map();               // nodeId -> { nodeIds:Set<string>, edgeIds:Set<string> }

function markSeedOnce() {
  if (seedElementIds.size) return;
  cy.elements().forEach(el => seedElementIds.add(el.id()));
}

function retainElements(col) {
  col.forEach(el => {
    if (seedElementIds.has(el.id())) return;
    ownerRefCount.set(el.id(), (ownerRefCount.get(el.id()) || 0) + 1);
  });
}

function releaseByIds(edgeIds = [], nodeIds = []) {
  // remove edges first
  edgeIds.forEach(id => {
    const el = cy.getElementById(id);
    if (el.empty() || seedElementIds.has(id)) return;
    const n = (ownerRefCount.get(id) || 1) - 1;
    if (n <= 0) { ownerRefCount.delete(id); el.remove(); } else ownerRefCount.set(id, n);
  });
  // then nodes
  nodeIds.forEach(id => {
    const el = cy.getElementById(id);
  ...
  // maintain clickedNodesHistory (keep your existing breadcrumb logic)
  ...
  const isExpanded = node.data('_expanded') === true;
  ...
  if (!isExpanded) {
    ...
    // Position new nodes around the anchor (avoid global relayout)
    const center = node.position();
    const newNodes = added.filter('node').filter(n => n.id() !== nodeId);
    const R = 90;
    ...
    const edgeIds = new Set(added.filter('edge').map(e => e.id()));
    expansionsByNode.set(nodeId, { nodeIds, edgeIds });
    retainElements(added);
    node.data('_expanded', true);
  } else {
    // ---------- COLLAPSE ----------
    const reg = expansionsByNode.get(nodeId);
    if (reg) {
      releaseByIds(Array.from(reg.edgeIds), Array.from(reg.nodeIds));
      expansionsByNode.delete(nodeId);
    }
