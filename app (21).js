...
cy.on('dbltap', 'node', async function(evt) {
        ...
        // --- Maintain breadcrumb UI (not used for API unless flag is flipped) ---
        ...
        let neighborsUrl = NEIGHBORS_API_URL_TEMPLATE.replace('{node_id}', nodeId) 
            + `?limit=10&node_type=${encodeURIComponent(nodeType)}&query_key=${encodeURIComponent(document.getElementById('current-query-key')?.value || '')}&months=${months}`;

        // Only append history if explicitly enabled
        if (USE_HISTORY_IN_NEIGHBOR_REQUEST && historyParams) {
            neighborsUrl += `&${historyParams}`;
        }

        // --- Expand: fetch, tag ownership, store for collapse, local fan-out ---
        const added = await fetchDataAndRender(neighborsUrl);
        calculateRelativeSizes();

        // Tag owners so we can collapse only what this node added
        const ownerId = nodeId;
        added.forEach(el => {
        ...
        // Save expansion state on the node
        node.scratch('_expanded', true);
        node.scratch('_expansionIds', added.map(e => e.id()));
        node.scratch('_expansionCol', added);
        node.addClass('expanded');

        // Local fan-out to avoid overlap/merge
        ...
        if (addedNodes.length > 0) {
            ...
            // Small jitter so identical positions never stack
            addedNodes.forEach(n => {
                const p = n.position();
                n.position({ x: p.x + (Math.random()-0.5)*4, y: p.y + (Math.random()-0.5)*4 });
            });

            // If many nodes, run a tiny local circle layout in a small box
            if (N >= 3) {
