...
function setActiveQueryKey(key){ const i=document.getElementById('current-query-key'); if(i){ i.value = key || ''; } }
    ...
    // ==== Per-node expansion registry for dbl-click toggle ====
    const __expansionByParent = new Map(); // parentId -> Set(elementIds)
...
cy.on('dbltap', 'node', async function(evt) {
    // Prevent the single-tap details panel from firing
    if (typeof tapTimeout !== 'undefined') clearTimeout(tapTimeout);
    ...
    // COLLAPSE: remove only what this node added previously
    if (__expansionByParent.has(nodeId)) {
        const ids = __expansionByParent.get(nodeId);
        const toRemoveEdges = [], toRemoveNodes = [];
        ids.forEach(eid => {
            const ele = cy.getElementById(eid);
            if (ele && ele.nonempty && ele.nonempty()) {
                if (ele.isEdge()) toRemoveEdges.push(ele);
            }
        });
        ...
        ids.forEach(eid => {
            const ele = cy.getElementById(eid);
            if (ele && ele.nonempty && ele.nonempty()) {
                if (ele.isNode() && ele.connectedEdges().length === 0) {
                    toRemoveNodes.push(ele);
                }
            }
        });
        cy.remove(cy.collection(toRemoveNodes));
        __expansionByParent.delete(nodeId);
        if (typeof reRunLayout === 'function') reRunLayout();
        if (typeof handleZoom === 'function') handleZoom();
        return;
    }

    // EXPAND (keep breadcrumb context, but don't drop earlier edges)
    const existingTypes = Object.keys(clickedNodesHistory);
    const idx = existingTypes.indexOf(nodeType);
    if (idx > -1) {
        const trimmed = {};
        for (let i = 0; i < idx; i++) trimmed[existingTypes[i]] = clickedNodesHistory[existingTypes[i]];
        clickedNodesHistory = trimmed;
    }
    ...
    let url = NEIGHBORS_API_URL_TEMPLATE.replace('{node_id}', nodeId)
        + `?limit=20&node_type=${encodeURIComponent(nodeType)}&query_key=${encodeURIComponent(qKey)}&months=${months}`;
    if (historyParams) url += `&${historyParams}`;

    // Run and record ONLY truly new elements; if nothing new, try once more (sometimes first fetch merges)
    let newIds = await __runExpansionAndGetNew(cy, async () => await fetchDataAndRender(url));
    if (newIds.length === 0) {
        // Retry once to avoid the "first dbl-click no-op" symptom
        newIds = await __runExpansionAndGetNew(cy, async () => await fetchDataAndRender(url));
    }

    if (newIds.length > 0) {
        __expansionByParent.set(nodeId, new Set(newIds));
        if (typeof calculateRelativeSizes === 'function') calculateRelativeSizes();
        if (typeof reRunLayout === 'function') reRunLayout();
        if (typeof handleZoom === 'function') handleZoom();
    } else {
        console.warn('No new neighbors for node', nodeId, 'type', nodeType);
    }
});
...
// === Ensure a default context-aware query is selected at startup ===
async function __ensureDefaultQuerySelection() {
  try {
    const input = document.getElementById('current-query-key');
    const hasKey = input && input.value && input.value.trim().length > 0;
    const defaultKey = 'client_account_payment_fi_prospect_ctx';
    if (!hasKey) {
      setActiveQueryKey(defaultKey);
      // fallback currentQuery so captions work
      if (typeof currentQuery !== 'object') currentQuery = {};
      currentQuery.caption_property = currentQuery.caption_property || 'name';
      currentQuery.edge_caption_property = currentQuery.edge_caption_property || 'name';
      const months = parseInt(document.getElementById('timescale-slider')?.value || '3');
      const url = SEARCH_API_URL_TEMPLATE.replace('{query_name}', defaultKey) + `?limit=25&months=${months}`;
      await fetchDataAndRender(url);
    }
  } catch (e) { console.warn('Default query selection skipped:', e); }
}
async function startApp() {
        showLoader();
        await updateHeaderInfo();
        await populateNav();
        await __ensureDefaultQuerySelection();
    }
