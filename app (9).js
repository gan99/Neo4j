...
// Smart placement for newly added neighbors to reduce overlap
function placeNeighborsAround(anchor, newNodes) {
  if (!newNodes || newNodes.length === 0) return;

  // Work in model coordinates using bounding box center
  ...
  // Estimate node size to pick spacing
  let avgSize = 0;
  newNodes.forEach(n => { avgSize += Math.max(n.width(), n.height()); });
  avgSize = (avgSize / Math.max(1, N)) || 30;

  const minSep = Math.max(30, avgSize * 0.6);
  // Ensure circumference can hold N nodes with min separation
  let baseRadius = 90;
  const neededRadius = (minSep * N) / (2 * Math.PI);
  let R = Math.max(baseRadius, neededRadius);

  if (N <= 12) {
    // Place on a ring
    const theta0 = Math.random() * Math.PI * 2; // slight randomness to avoid exact overlaps
    newNodes.positions((i, n) => {
      const theta = theta0 + (2 * Math.PI * i) / N;
      return { x: cx + R * Math.cos(theta), y: cy + R * Math.sin(theta) };
    });
  } else {
    // Use a golden-angle spiral for large N
    const golden = 2.399963229728653; // ~137.5 degrees
    const step = minSep * 0.9;
    const theta0 = Math.random() * Math.PI * 2;
    newNodes.positions((i, n) => {
      const r = R + i * (step * 0.5);
      const theta = theta0 + i * golden;
      return { x: cx + r * Math.cos(theta), y: cy + r * Math.sin(theta) };
    });
  }

  // For very large batches, nudge with a small local layout within a bounding box
  if (N > 25) {
    const pad = Math.max(200, R + 200);
    const subgraph = newNodes.union(newNodes.connectedEdges());
    subgraph.layout({
      name: 'fcose',
      animate: true,
      randomize: true,
      ...
      idealEdgeLength: 120,
      fit: false,
      boundingBox: { x1: cx - pad, y1: cy - pad, x2: cx + pad, y2: cy + pad }
    }).run();
  }
}
