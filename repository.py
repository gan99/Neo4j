            ...
            for _, value in record.items():
                ...
                if hasattr(value, 'labels'): # It's a Node
                    ...
                    if node_id not in nodes:
                        # Choose a display name for the node.  Prefer a "name" property if present,
                        # otherwise find any property whose key contains "name", then one containing
                        # "id", and finally fall back to the first string property.  If none of
                        # those exist, default to "Unnamed".
                        caption = None
                        # 1) direct 'name' key
                        if value.get("name"):
                            caption = value.get("name")
                        # 2) any key containing 'name'
                        if not caption:
                            for k, v in value.items():
                                if k.lower().find("name") != -1 and v is not None:
                                    caption = v
                                    break
                        # 3) any key containing 'id'
                        if not caption:
                            for k, v in value.items():
                                if k.lower().find("id") != -1 and v is not None:
                                    caption = v
                                    break
                        # 4) fallback to first string property
                        if not caption:
                            for k, v in value.items():
                        ...
                        # Copy all properties from the Neo4j node to the cytoscape node data.  This
                        # allows custom caption properties such as `party_name` to be referenced in
                        # the frontend.
                        for k, v in value.items():
                            # Skip any reserved keys that we have already set; otherwise include.
                            if k not in node_data:
                                node_data[k] = v
                        # Apply optional size and community mappings.
