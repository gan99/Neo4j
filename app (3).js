...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function parseElements(elements, anchorNode) {
        ...
        if (anchorNode && newNodes.length > 0) {
            /*
             * Position newly added nodes in concentric rings around the anchor node.
             * This approach reduces overlap by distributing neighbours across
             * multiple rings when there are many of them.  Each ring holds up
             * to `maxPerRing` nodes.  The radius grows by `baseRadius +
             * additionalSpacing` for each successive ring.
             */
            const baseRadius = Math.max(parseInt(edgeLengthSlider.value), 50);
            ...
            newNodes.forEach(n => {
                ...
                const nodesRemaining = newNodes.length - ringIndex * maxPerRing;
                const nodesInThisRing = Math.min(maxPerRing, nodesRemaining);
                const angleStep = (2 * Math.PI) / nodesInThisRing;
                const angle = angleStep * posInRing;
                const radius = baseRadius + ringIndex * (baseRadius + additionalSpacing);
    ...
    function expandNode(node) {
        ...
        fetchDataAndRender(neighborsUrl, node).then(() => {
            /*
             * We deliberately avoid restarting the global simulation here.  New
             * nodes are placed in rings around the anchor node, and restarting
             * the force layout would cause them (and their neighbours) to
             * drift away.  This keeps existing nodes in place regardless of
             * the selected layout.
             */
        });
    }
