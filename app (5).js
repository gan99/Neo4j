...
// ===== Expand/Collapse (dbltap toggle) helpers =====
function buildNeighborsUrl(node) {
  const nodeId   = node.id();
  const nodeType = node.data('label');
  const queryKey = document.getElementById('current-query-key')?.value || '';
  let url = NEIGHBORS_API_URL_TEMPLATE
              .replace('{node_id}', nodeId)
            + `?limit=12&node_type=${encodeURIComponent(nodeType)}&query_key=${encodeURIComponent(queryKey)}`;
  if (typeof getHistoryParams === 'function') {
    const h = getHistoryParams();
    if (h) url += `&${h}`;
  }
  return url;
}

function getAddedSince(beforeIds) {
  return cy.elements().filter(ele => !beforeIds.has(ele.id()));
}

function placeAround(anchorNode, newNodes) {
  const center = anchorNode.position();
  const R = 100;
  const N = Math.max(1, newNodes.length);
  newNodes.positions((i, n) => {
    const theta = (2 * Math.PI * i) / N;
    return { x: center.x + R * Math.cos(theta), y: center.y + R * Math.sin(theta) };
  });
}

function addOwnerTags(eles, ownerId) {
  eles.forEach(ele => {
    const owners = Array.isArray(ele.data('_owners')) ? ele.data('_owners') : [];
    if (!owners.includes(ownerId)) {
      owners.push(ownerId);
      ele.data('_owners', owners);
    }
  });
}

function removeOwnedOrphanElements(eles, ownerId) {
  const toRemove = [];
  eles.forEach(ele => {
    const owners = Array.isArray(ele.data('_owners')) ? ele.data('_owners') : [];
    const nextOwners = owners.filter(o => o !== ownerId);
    if (nextOwners.length === 0) {
      if (ele.id() !== ownerId) toRemove.push(ele);
      ele.removeData('_owners');
    } else {
...
function installExpandToggle() {
  if (!window.cy) return;
  try {
    if (typeof cy.off === 'function') {
      cy.off('dbltap'); // remove previous dbltap handlers if any
    }
  } catch (e) {}
  ...
  cy.on('dbltap', 'node', async function (evt) {
    ...
    if (node.scratch('_expanded')) {
      const added = node.scratch('_expansionEles');
      if (added && added.length) {
        removeOwnedOrphanElements(added, nodeId);
      }
      node.scratch('_expanded', false);
      node.scratch('_expansionEles', null);
    ...
    try {
      ...
      if (typeof fetchDataAndRender === 'function') {
        ...
        console.error('fetchDataAndRender() not found');
      }

      const added = getAddedSince(beforeIds);
      const addedNodes = added.nodes().filter(n => n.id() !== nodeId && !n.locked());

      if (typeof calculateRelativeSizes === 'function') calculateRelativeSizes();
      placeAround(node, addedNodes);
      addOwnerTags(added, nodeId);

      node.scratch('_expanded', true);
      node.scratch('_expansionEles', added);
...
// wait for cy to exist
(function waitForCyAndInstall(){
  if (window.cy) {
    installExpandToggle();
  } else {
    window.addEventListener('load', () => {
      if (window.cy) installExpandToggle();
      else {
        const iv = setInterval(() => {
          if (window.cy) { clearInterval(iv); installExpandToggle(); }
        }, 200);
      }
    });
  }
})();
// ===== End Expand/Collapse helpers =====
